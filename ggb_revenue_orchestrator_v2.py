#!/usr/bin/env python3
"""GGB Revenue Orchestrator - Sales Goal Agent (Optimized Version)
Published: 48/94 | Unpublished: 46
Revenue: $0.00 / $10,000.00
"""
import json
import os
import sys
...
print("=" * 60)
print("GGB REVENUE ORCHESTRATOR - OPTIMIZED RUN")
print("=" * 60)
...
if unpublished:
    ...
    for i, p in enumerate(unpublished):
        ...
        try:
            ...
            if e.code == 429:
                wait_time = 120  # Increased wait time
        ...
        if consecutive_failures >= 2:
            print(f"    Too many failures, backing off for 180s...")
            time.sleep(180)
            consecutive_failures = 0
