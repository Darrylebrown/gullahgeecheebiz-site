#!/usr/bin/env python3
"""GGB Agent B-7 — Substack Executor. Magazines, newsletters to Substack."""
import json, sys
from pathlib import Path
sys.path.insert(0, str(Path(__file__).resolve().parent.parent))
from publisher import PublishEngine, build_canonical_manifest_hash, REPO_ROOT
AGENT_NAME, AGENT_VERSION, PLATFORM = "agent-b-substack", "1.0.0", "substack"
...
class SubstackExecutor:
    ...
    def upload_article(self, mid):
        a=self.verify_approval(mid)
        if not a["verified"]: return {"error": a["error"]}
        m=self.engine.db.load_manifest(mid)
        if not self.engine.adapter.check_auth().get("authenticated"): return {"error":"Substack auth failed"}
        did=m.get("draft_id","new-substack")
        content_dir=REPO_ROOT/"publish"/"magazines"
        ups=[]
        for f in sorted(content_dir.glob("*.html")):
            if mid[:8] in f.name:
                r=self.engine.adapter.upload_artifact(did,"article",str(f))
                if r.get("success"): ups.append({"file":f.name})
        return {"status":"uploaded","platform":PLATFORM,"draft_id":did,"articles_uploaded":len(ups),"_mock":self.engine.adapter.is_mock()}
    ...
    def submit(self, mid, owner_approved=False):
        ...
        r=self.engine.adapter.submit(m.get("draft_id","new-substack"))
...
if __name__=="__main__":
    ...
    a=p.parse_args(); e=SubstackExecutor()
    if a.command=="upload": r=e.upload_article(a.manifest_id)
