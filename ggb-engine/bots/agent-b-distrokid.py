#!/usr/bin/env python3
"""GGB Agent B-4 — DistroKid Executor. Music to Spotify/Apple/TikTok."""
import json, sys, time
from pathlib import Path
sys.path.insert(0, str(Path(__file__).resolve().parent.parent))
from publisher import PublishEngine, build_canonical_manifest_hash, REPO_ROOT
AGENT_NAME, AGENT_VERSION, PLATFORM = "agent-b-distrokid", "1.0.0", "distrokid"

class DistroKidExecutor:
    ...
    def upload(self, mid):
        ...
        m=self.engine.db.load_manifest(mid)
        if not self.engine.adapter.check_auth().get("authenticated"): return {"error":"DistroKid auth failed"}
        did=m.get("draft_id","new-distro")
        audio_dir=REPO_ROOT/"publish"/"audio"
        ups=[]
        for f in sorted(audio_dir.glob("*.mp3")):
            if mid[:8] in f.name:
                for _ in range(self.max_retries):
                    r=self.engine.adapter.upload_artifact(did,"audio",str(f))
                    if r.get("success"): ups.append({"file":f.name}); break
                    time.sleep(3)
        return {"status":"uploaded","platform":PLATFORM,"draft_id":did,"files_uploaded":len(ups),"_mock":self.engine.adapter.is_mock()}
    ...
    def submit(self, mid, owner_approved=False):
        ...
        r=self.engine.adapter.submit(m.get("draft_id","new-distro"))
...
if __name__=="__main__":
    ...
    a=p.parse_args(); e=DistroKidExecutor()
    if a.command=="upload": r=e.upload(a.manifest_id)
