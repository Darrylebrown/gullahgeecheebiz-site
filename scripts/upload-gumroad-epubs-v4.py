...
def upload_file_presign(epub_path):
    ...
    presign_data = result.get("file", {})
    s3_url = presign_data.get("url")
    parts = presign_data.get("parts", [])
    
    if not s3_url:
        return None, "No presign URL returned"
    
    log(f"  Got presign URL, parts: {len(parts)}")
    
    # Step 2: Upload to S3
    with open(epub_path, 'rb') as f:
        file_data = f.read()
    
    # For small files, use single part upload
    if len(parts) == 1 or filesize < 10 * 1024 * 1024:  # < 10MB
        log(f"  Uploading single part to S3...")
        r = requests.put(s3_url, data=file_data, timeout=120)
        if r.status_code not in [200, 204]:
            return None, f"S3 upload failed: {r.status_code}"
        
        etag = r.headers.get('ETag', '').strip('"')
        log(f"  S3 upload successful, ETag: {etag}")
        
        # Step 3: Complete
        complete_result = api_call("POST", "files/complete", data={
        ...
        file_url = complete_result.get("file", {}).get("url")
        log(f"  File URL: {file_url}")
        return file_url, None
    else:
        # Multi-part upload (not needed for our small EPUBs)
        return None, "Multi-part upload not implemented"
...
def main():
    ...
    log("=" * 60)
    log("Gumroad EPUB Upload V4 (Presign Flow)")
    log("=" * 60)
