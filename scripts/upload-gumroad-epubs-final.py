...
Fixed: Use files[][url] format for attachment.
"""
...
LOG_FILE = LOG_DIR / "upload_epub_final.log"
...
def api_call(method, endpoint, data=None, files=None, json_data=None, headers=None):
    url = f"{API}/{endpoint}"
    ...
    for attempt in range(3):
        try:
            if method == "GET":
                ...
                r = requests.post(url, params=params, data=data, files=files, json=json_data, headers=request_headers, timeout=60)
...
def get_product(pid):
    """Get full product details including files."""
...
def upload_file_presign(epub_path):
    ...
    # Step 3: Complete multipart upload - use JSON body
    complete_data = {
        "upload_id": upload_id,
        "key": key,
        "parts": part_etags
    ...
    complete_result = api_call("POST", "files/complete", json_data=complete_data,
...
def main():
    ...
    # Track which products already have files
    existing_files = {}
    for pid, product in products.items():
        full_product = get_product(pid)
        files = full_product.get("files", [])
        existing_files[pid] = len([f for f in files if f.get("url")])
    ...
    for pid, product in products.items():
        ...
        if existing_files.get(pid, 0) > 0:
            log(f"SKIP {name}: Already has {existing_files[pid]} file(s)")
            skipped += 1
            continue
        ...
        # Attach file to product using PUT with files[][url] format
    ...
    # Save matches
    with open(LOG_DIR / "matches_final.json", "w") as f:
        json.dump(matches, f, indent=2)
    log(f"Saved {len(matches)} matches to matches_final.json")
    
    # Verify - get fresh product list
    log("\nVerifying products...")
    products = get_products()
    ready_count = 0
    for pid, product in products.items():
        ...
        full_product = get_product(pid)
        files = full_product.get("files", [])
        file_count = len([f for f in files if f.get("url")])
