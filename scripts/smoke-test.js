...
console.log('Gulfill Geechee Biz — Smoke Test\n' + '='.repeat(40));

// 1. Key HTML files exist
const htmlFiles = [
  'index.html', 'shop.html', 'shop-binyah.html', 'bot-dashboard.html',
  'membership/index.html', 'season-1/index.html', 'guide/index.html',
  'services/index.html', 'books.html'
];
...
} catch (e) { fail('sitemap', e.message); }
...
} catch (e) { fail('.gitignore', 'missing'); }

// 6. node_modules NOT tracked
try {
...
} catch (e) { ok('git: node_modules not tracked'); }

// 7. No tokens in git remotes
try {
...
} catch (e) { fail('git: remotes', e.message); }

// 8. deploy-bot.py syntax
try {
  execSync('python3 -c "import ast; ast.parse(open(\'scripts/deploy-bot.py\').read())"', { cwd: ROOT, encoding: 'utf8', stdio: 'pipe' });
  ok('deploy-bot.py: syntax valid');
} catch (e) { fail('deploy-bot.py: syntax valid', e.stderr || e.message); }

// 9. build-membership.py syntax
try {
  execSync('python3 -c "import ast; ast.parse(open(\'scripts/build-membership.py\').read())"', { cwd: ROOT, encoding: 'utf8', stdio: 'pipe' });
  ok('build-membership.py: syntax valid');
} catch (e) { fail('build-membership.py: syntax valid', e.stderr || e.message); }
...
try {
  const ogBroken = execSync("grep -rl 'og-image.jpg' --include='*.html' . 2>/dev/null | grep -v node_modules | wc -l", { cwd: ROOT, encoding: 'utf8' }).trim();
