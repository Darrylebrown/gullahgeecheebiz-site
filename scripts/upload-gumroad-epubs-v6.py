...
Fixed: correct parameter names and response parsing.
"""
...
def upload_file_presign(epub_path):
    ...
    # Response has upload_id, key, file_url, parts directly
    ...
    # Use the first part's presigned URL
    if parts:
        s3_url = parts[0].get("presigned_url")
        if not s3_url:
            return None, "No presigned URL in parts"
...
def main():
    ...
    log("=" * 60)
    log("Gumroad EPUB Upload V6 (Presign Flow Fixed)")
    log("=" * 60)
